...
function repeat(str, targetBytes) {
...
function bench(label, sizeBytes, targetMs) {
  const input = repeat(CHUNK, sizeBytes)
  const RUNS = 5

  engine.scan(input, { mode: ReplacementMode.Redact }) // warmup
  ...
  return { label, sizeKb: Math.round(sizeBytes / 1024), ms: Math.round(avg), detections, pass: avg < targetMs, target: targetMs }
}
...
for (const r of results) {
  ...
  console.log(`${status}  ${r.label}  ${r.ms}ms / ${r.target}ms target  (${r.detections} detections, ${r.sizeKb}KB)`)
